...
export function PostForm({ post }: PostFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.publishedAt) {
      submitData.append("publishedAt", formData.publishedAt.toISOString());
    }
    ...
    <Card className="rounded-2xl border-none shadow-xl bg-white">
      <CardContent className="p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Published Date */}
          <div className="space-y-2">
            <Label htmlFor="publishedAt" className="text-[#1E3A8A] font-semibold">
                Published Date
            </Label>
            <Popover>
                <PopoverTrigger asChild>
                    <Button
                        variant={"outline"}
                        className={cn(
                            "w-full justify-start text-left font-normal h-12 rounded-lg border-[#BFDBFE] focus:border-[#3B82F6] focus:ring-[#3B82F6]",
