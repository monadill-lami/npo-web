...
export async function listPostsSummary(): Promise<Array<{ id: string; title: string; image: string | null; timestamp: string }>> {
  ...
  try {
    for await (const [_key, value] of db.iterator({ gte: postsPrefix, lt: 'post;' })) {
      ...
      list.push({ id: p.id, title: p.title, image, timestamp: p.publishedAt || p.createdAt });
    }
  } catch (err: any) {
...
export async function updatePost(
  id: string,
  updates: Partial<Pick<PostRecord, 'title' | 'subTitle' | 'description' | 'imageUrl' | 'imagePath' | 'publishedAt'>>
